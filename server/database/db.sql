...
CREATE TABLE IF NOT EXISTS `neuss`.`CourseOrder` (
  ...
  `IdCourseOrder` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idCourse`, `idOrder`, `IdCourseOrder`),
  INDEX `fk_Course_has_Order_Order1_idx` (`idOrder` ASC) VISIBLE,
  INDEX `fk_Course_has_Order_Course1_idx` (`idCourse` ASC) VISIBLE,
  CONSTRAINT `fk_Course_has_Order_Course1`
    FOREIGN KEY (`idCourse`)
    REFERENCES `neuss`.`Course` (`idCourse`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Course_has_Order_Order1`
    FOREIGN KEY (`idOrder`)
